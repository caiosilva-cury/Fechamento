...
while True:
    ...
    try:
        dt_fechamento = datetime.strptime(entrada, "%Y-%m-%d").date()
        print(f"\n✅ Data de fechamento definida: {dt_fechamento}")
        break
    except ValueError:
...
with engine.begin() as conn:
    resultado = conn.execute(
        text(f"SELECT COUNT(*) FROM {TABELA_SQL} WHERE dt_fechamento = :dt"),
        {"dt": dt_fechamento}
    ).scalar()

if resultado > 0:
    print(f"\n⚠️  Atenção: já existem {resultado:,} registros para {dt_fechamento} na tabela.")
    print("   Se continuar, os dados existentes serão DELETADOS e substituídos.")
    confirmacao = input("\n   Deseja continuar? (S/N): ").strip().upper()
    if confirmacao != "S":
        print("\n❌ Importação cancelada pelo usuário.")
        exit()
    print(f"\n🧹 Deletando registros existentes de {dt_fechamento}...")
    with engine.begin() as conn:
        conn.execute(
            text(f"DELETE FROM {TABELA_SQL} WHERE dt_fechamento = :dt"),
            {"dt": dt_fechamento}
        )
...
# =====================================================
# INSERIR COLUNA dt_fechamento NA PRIMEIRA POSIÇÃO
# =====================================================

df.insert(0, "dt_fechamento", dt_fechamento)
...
print(f"{'=' * 55}")
print(f"  Competência : {dt_fechamento}")
